...
拠点はグローバルな設定ではなく、いま見ているリソースの一部だからだ。GitHub のリポジトリと同じ立ち位置。Cookie に持つと、2つのタブで別々の拠点を開けず、片方のタブで切り替えるともう片方のタブの取得先が黙って変わってしまう。
...
## 規格はバージョン管理され、使われたら不変

配置エリアは規格バージョンの系列を持つ。あるバージョンが配置決めに使われたら、それ以降は変更・削除・公開取り消しができない。編集は新しいバージョンへの複製を意味する。

配置決めは、その日付時点で有効だったバージョンを解決する。したがって新しく公開したバージョンが過去日の見え方を書き換えることはない。「先週の月曜、誰がどこにいたか」は固定される。

シフトも同じ原則に従う。物理削除ではなくソフト削除するため、過去の配置決めは当時のシフト名を参照し続けられる。
