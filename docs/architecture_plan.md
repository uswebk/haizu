...
| データベース       | PostgreSQL                                  |
| 認証認可         | Better Auth                                 |
| リアルタイム配信     | （MVP 対象外・保留。実装時の方式は SSE 予定）                 |
| オブジェクトストレージ  | Cloudflare R2（S3 互換、図面アップロード用）              |
| キャッシュ/PubSub | （リアルタイム配信に付随。MVP では不要）                      |
...
- `[確定]` **コンピュート: Cloud Run 主体**（フロント・バックを各コンテナ、scale-to-zero）。
- `[確定]` **DB: Neon**（サーバーレス PostgreSQL、無料枠 + scale-to-zero。Cloud Run から TCP 接続）。
- `[確定]` **ストレージ: Cloudflare R2**（egress 無料、S3 互換）。
- `[確定]` **前段に Cloudflare**（CDN / DNS / DDoS、無料プラン）。
- `[確定]` 初期コスト方針: **ほぼ無料で運用**（Cloud Run 無料枠 + Neon 無料枠 + Cloudflare 無料 + R2 従量）。
- `[暫定]` コールドスタートが問題化した場合のみ Cloud Run `min-instances=1` を検討。
...
- `[未決]` リアルタイム配信（モニターへのアナウンス即時表示）は **MVP 対象外として保留**。配置ビュアーは MVP では定期更新/手動更新で表示し、即時プッシュは将来設計する。実装時の方式は **SSE（一方向プッシュ）を予定**、ファンアウト手段（Redis / Postgres LISTEN/NOTIFY）も同時に決める。
...
- 認証認可 / アカウント管理（権限 4 種）
- 従業員登録（CSV 取り込み、アバターカラー等）
- 配置規格登録（図面アップロード、人員・設備の D&D 配置、バージョン管理）
- 配置決め（日付・交代制対応、従業員アサイン）
- 配置履歴（過去実績の確認、メモ、CSV エクスポート）
- 配置ビュアー（モニター表示専用、操作不可、アナウンス表示）
- 管理者用設定 / 拠点登録（複数拠点を 1 事業所で管理）
